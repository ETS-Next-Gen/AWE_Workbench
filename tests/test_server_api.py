...
class ServerAPITest(unittest.TestCase):
    ...
    def testSearchPhrase(self):
        ok = self.parser.send(['PARSEONE', 'test', 'A dog barked at a cat'])
        self.assertEqual(ok, True)
    ...
    def testSearchPhrase2(self):
        ok = self.parser.send(['PARSESET',[labels,texts]])
        self.assertEqual(ok, True)
        ok = self.parser.send(['NEWSEARCHPHRASE','A mouse helps a lion', 'ml'])
        ok = self.parser.send(['NEWSEARCHPHRASE','Technology encourages thinking', 'tt'])
        ok = self.parser.send(['NEWSEARCHPHRASE','No man saw me', 'nm'])
    ...
    def testTopicMatches(self):
        ok = self.parser.send(['PARSESET',[labels,texts]])
        self.assertEqual(ok, True)
        matches = self.parser.send(['TOPIC_MATCHES','A mouse helps a lion.'])
