        ...
        async for message in websocket:
            ...
            try:
                ...
                #      building.
                    ...
                    doc = self.parser.get_document(label)
                    await websocket.send(
                        json.dumps(doc._.paragraph_lengths))
                    ...
                    doc = self.parser.get_document(label)
                    await websocket.send(json.dumps(
                        doc._.sliding_window_cohesions))
                    ...
                    doc = self.parser.get_document(label)
                    await websocket.send(json.dumps(
                        doc._.weightedSyntacticBreadths))
                    ...
                    doc = self.parser.get_document(label)
                    await websocket.send(json.dumps(
                        doc._.vwp_statements_of_fact))
                elif messagelist[0] == 'STATEMENTSOFOPINION':
                    # list of positions where source is indicated
                    label = messagelist[1]
                    doc = self.parser.get_document(label)
                    await websocket.send(json.dumps(
                        doc._.vwp_statements_of_opinion))
                    ...
                    # which corresponds to the words in the spans to which
                    ...
                    # lexicon/code. Returns a dictionary with four top
                    # level keys:
                    #
                    # 1.Implicit: Spans whose perspective is implicitly that of
                    #   the speaker/author
                    ...
                    #
                    # Each of these keys is associated with a list of offsets,
                    # whichcorresponds to the stance markers in the spans to
                    ...
                    # token offsets in the document that are part of a claim
                    # sentence
                    label = messagelist[1]
                    doc = self.parser.get_document(label)
                    ...
                    # Returns a sequence of true/false flags corresponding to
                    # token offsets in the document that are emotion words
                    label = messagelist[1]
                    doc = self.parser.get_document(label)
                    ...
                    # Returns a sequence of true/false flags corresponding
                    # to token offsets in the document that are character words
                    label = messagelist[1]
                    doc = self.parser.get_document(label)
                    ...
                    #
                    # Each of these keys is associated with a list of offsets,
                    # which corresponds to character trait predicates that
                    ...
                    # Returns a dictionary with four top level keys:
                    # 1.Implicit: Spans whose perspective is implicitly
                    # that of the speaker/author
                    # 2. First person: Spans whose perspective is explicitly
                    #    that of the speaker/author, indicated by first person
                    #    pronouns.
                    # 3. Second person: Spans whose perspective is explicitly
                    ...
                    #
                    # Each of these keys is associated with a list of offsets,
                    # which corresponds to propositional attitude predicates
                    # like think, feel, or believe, that describe mental states
                    # of the entity referred to
                    label = messagelist[1]
                    doc = self.parser.get_document(label)
                    await websocket.send(json.dumps(
                        doc._.vwp_propositional_attitudes))
                elif messagelist[0] == 'SOCIAL_AWARENESS':
                    command = 'SOCIAL_AWARENESS'
                    label = messagelist[1]
                    doc = self.parser.get_document(label)
                    await websocket.send(json.dumps(
                        doc._.vwp_social_awareness))
                    ...
                    # Returns a list of offsets to tokens that cue
                    # a more oral, interactive style
                    label = messagelist[1]
                    doc = self.parser.get_document(label)
                    ...
                    doc = self.parser.get_document(label)
                    await websocket.send(json.dumps(
                        doc._.vwp_explicit_arguments))
                elif messagelist[0] == 'SUBJECTIVITYRATINGS':
                    command = 'SUBJECTIVITYRATINGS'
                    # Real number between 0 and 1: tokens that cue
                    # subjectivity of the text (stronger or weaker
                    # stance). Position in the list returned equals
                    # position in the document
                    label = messagelist[1]
                    doc = self.parser.get_document(label)
                    await websocket.send(json.dumps(
                        doc._.subjectivity_ratings))
                elif messagelist[0] == 'SENTIMENTRATINGS':
                    command = 'SENTIMENTRATINGS'
                    # 1 or 0: tokens that cue positive or negative
                    # sentiment polarity. Position in the list returned
                    ...
                    # Real number betweeen -1 and 1: tokens that cue
                    # positive or negative TextBlob sentiment polarity.
                    # Position in the list returned equals position
                    # in the document
                    label = messagelist[1]
                    doc = self.parser.get_document(label)
